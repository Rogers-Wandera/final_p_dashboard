...
const Maintenance = () => {
  useEffect(() => {
    ...
    function getTimeRemaining(endtime: any) {
      const total =
        Date.parse(endtime) - Date.parse(new Date() as unknown as string);
    ...
    function initializeClock(elem: any, endtime: any) {
    ...
    if (time === undefined) {
      time = (Date.parse(new Date() as unknown as string) +
        15 * 24 * 60 * 60 * 1000) as unknown as string;
    }
    const deadline = new Date(time as unknown as string);
